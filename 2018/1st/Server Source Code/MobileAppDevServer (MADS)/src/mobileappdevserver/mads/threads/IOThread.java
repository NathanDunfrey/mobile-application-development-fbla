...
import mobileappdevserver.mads.Log;
...
public class IOThread implements Runnable {
    ...
    @Override public void run() {
        ...
        while(Server.getInstance().isRunning()) {
            try {
                int i = in.read(buf);
                if(i == -1 || i == 0) {
                        throw new RuntimeException("Disconnected: " + this);
                }
    ...
    public void addBookToStack(Book b) {
        ...
        addMessage(b.getPublishYear());
        addMessage(b.getPublisher());
    }
    ...
    public void flush() {
        synchronized(outStackLock) {
            try {
                ...
                out.write(output);
                out.flush();
                Log.m("Wrote " + new String(output) + " to " + this);
                outStack.close();
                outStack = new ByteArrayOutputStream();
            } catch(IOException e) {
                e.printStackTrace();
    ...
    public String toString() {
        if(nameInfo == null) {
            ...
            return nameInfo[0] + " " + nameInfo[1] + " (" + socketID + ")";
        }
