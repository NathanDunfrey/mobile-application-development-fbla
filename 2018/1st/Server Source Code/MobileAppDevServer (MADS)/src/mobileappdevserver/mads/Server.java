...
public class Server {
    ...
    public void processInput(byte[] in, long socketID, IOThread client) {
        Log.m(client.toString() + ": " + new String(in));
        Log.m("("+in.length+")");
        ...
        if(!client.isVerified()) {
            ...
            try {
                ...
                if(spl[0].equalsIgnoreCase("search")) {
                    ...
                    client.flush();
                } else if(spl[0].equalsIgnoreCase("return")) {
                    String isbn = spl[1];
                    String userAbout = spl[2];
                     if(client.getOccupation().equalsIgnoreCase("Administrator")) {
                         String ret = "";
                         try{
                            ret = db.returnCheckedOut(userAbout, isbn);
                            if(ret == null) {
                                client.addMessage("accepted");
                            } else {
                                client.addMessage(ret);
                            }
                         } catch(Exception e) {
                             e.printStackTrace();
                             client.addMessage("error");
                   ...
                   client.flush();
                    ...
                    client.addMessage("accepted");
                    String[] name = client.getNameInfo();
                        client.addMessage(name[0]);
                        client.addMessage(name[1]);
                        client.addMessage(name[2]);
    ...
    public void newConnection(IOThread io) {
        new Thread(io).start();
        cons.add(io);
        Log.m("Connection from " + io.getIP());
    }
    public void disconnect(IOThread io) {
        cons.remove(io);
        Log.m("Disconnection from " + io.getIP());
    }
